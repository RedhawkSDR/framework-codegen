...
bool ${className}::loadHardware(HwLoadStatusStruct& requestStatus) 
{
    // The hardware may be physically loaded at this point
    return true;
}

void ${className}::unloadHardware(const HwLoadStatusStruct& requestStatus) 
{
    // The hardware may be physically unloaded at this point
}

${executeClass}* ${className}::generate${executeType.capitalize()}(int argc, char* argv[], ConstructorPtr ${executeType}EntryPoint, const char* libName) 
{
    return ${executeType}EntryPoint(argc, argv, this);
}
